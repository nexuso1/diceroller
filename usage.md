...
- `((d20 max d20 + n - 5 >= k) * (2d6 + 5 + 10)) >= ((d20 max d20 + n >= k) * (2d6 + 5 ))` Comparison between attacks with Heavy Weapon Master(HWM) and without. `n` is the base attack bonus, and `k` is the AC. The median matrix will have `1` on the positions where the HWM attack deals more (or equal) damag on average. Use variables to set different values for `n` and `k`.